  ...
  return (
    <Panel as="section" className="p-5 sm:p-6">
      <form className="space-y-4" onSubmit={handleSubmit}>
        <h2 className="font-display text-xl font-semibold text-ink">{submitLabel}</h2>

        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Title" error={errors.title}>
            <input className="input !rounded-xl" value={title} onChange={(e) => setTitle(e.target.value)} />
          </Field>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="External link" error={errors.externalUrl}>
            <input className="input !rounded-xl" value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} />
          </Field>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Location" error={errors.venue}>
            <input className="input !rounded-xl" value={venue} onChange={(e) => setVenue(e.target.value)} />
          </Field>
          <Field label="City" error={errors.city}>
            <input className="input !rounded-xl" value={city} onChange={(e) => setCity(e.target.value)} />
          </Field>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Latitude" error={errors.latitude}>
            <input className="input !rounded-xl" value={latitude} onChange={(e) => setLatitude(e.target.value)} />
          </Field>
          <Field label="Longitude" error={errors.longitude}>
            <input className="input !rounded-xl" value={longitude} onChange={(e) => setLongitude(e.target.value)} />
          </Field>
        </div>
